...
class ConvexProximalMethod():
    ...
    def prox_G1_KL(self, theta):
        ...
        # print((self.mu * np.nan_to_num(1 / sum_theta).reshape(-1, 1)).shape)
    ...
    def f_G2(self, nu, theta):
        # nu vecteur vertical
        # print("test", nu.shape, self.phi.shape, (self.phi.T @ nu).shape)
    ...
    def jacobian_f_G2(self, nu, theta):
        ...
        #print("Helloooo", A.shape, np.tile(A, (1, nu.size)))
        return np.eye(nu.size) + ((np.eye(nu.size) + self.phi) / self.eps) * np.tile(A, (1, nu.size))

    def prox_G2_KL(self, theta):  
        assert np.sum(self.phi ** 2) < 1 #TODO remplacer par une erreur
        #old_nu = np.maximum(np.sum(theta, axis=0), (10 ** -300)) 
        ...
        # print("shape", nu.shape, old_nu.shape)
        return theta * np.exp(-(nu + self.phi.T @ nu) / self.eps) / np.sum(theta, axis=0)
    ...
    def jacobian_f_G3(self, nu, theta):
        ...
        A = np.sum(theta, axis=0).reshape(nu.shape) * np.exp(- self.h_G3(nu) / self.eps)
        return np.eye(nu.size) + (1 / self.eps) * np.diag((self.hprim_G3(nu) * A).ravel()) #np.eye(nu.size) + self.hprim_G2(self.mu) * np.tile(A, (1, nu.size))

    def prox_G3_KL(self, theta):
        #nu = newton(self.f_G3, np.zeros((theta.shape[1], 1)), self.jacobian_f_G3, self.eps, args=(theta,))
        #old_nu = np.maximum(np.sum(theta, axis=0), (10 ** -300)) 
        nu = dichotomy(self.f_G3, np.zeros((theta.shape[1], 1)), np.ones((theta.shape[1], 1)), PREC, args=(theta,))
        return theta * (np.exp(- self.h_G3(nu) / self.eps)) / np.sum(theta, axis=0)
    ...
    def fit(self):
        ...
        while not break_loop and n < 11: # todo : trouver une condition sur le gap qui ne soit pas dépendante du nombre de points
            print(n, flush=True)
...
class ConvexProximalMethod2():
    ...
    def prox_G2_KL(self, theta):  
        ...
        # nu = dichotomy(self.f_G2, np.zeros((theta.shape[1], 1)), np.ones((theta.shape[1], 1)), PREC, args=(theta,))
        # print("shape", nu.shape, old_nu.shape)
        return theta * np.exp(-(PEN_DENSITY * nu + self.phi.T @ nu) / self.eps) / np.sum(theta, axis=0)
    ...
    def fit(self):
        ...
        break_loop = False
        # print("gamma", np.linalg.norm(gamma))
        while not break_loop and n < 20: # todo : trouver une condition sur le gap qui ne soit pas dépendante du nombre de points
            print(n, flush=True)
...
if __name__ == "__main__":
    ...
    if mode == "reprod":
        ...
        phi = (10 ** -6) * (y.reshape(-1,1) - y.reshape(1, -1)) ** 2
        # print("B", phi.shape)
        ...
        def ComputeGamma(max_attractiveness, reward_attractiveness, pen_overcost, pen_density, pen_distance, n_points, epsilon, l_intervalle):
            ...
            #+ np.tile(potential, (x.size, 1))
            def h_G3(nu): # fonction issue du potentiel
                return pen_density * nu #(nu ** 2)
            
            def hprim_G3(nu):
                return pen_density * nu#56 * (nu ** 6)

            method = ConvexProximalMethod2(mu, costs, phi, EPS, h_G3, hprim_G3, E)
            gamma, nu = method.fit()
            return gamma, nu, mu, x, y, attract_y, costs, phi
        ...
        def resume(gamma):
            ...
            print("apport attractivite moyen : ", MAX_ATTRACTIVENESS * (1-attract_y) @ nu)
            print("penalisation distance moyenne: ", PEN_DISTANCE * np.mean(((y.reshape(-1, 1) - y.reshape(1, -1)) ** 2) @ nu) )
    ...
    if mode == "debug" :
        ...
        for a in range(1, 10):
            plt.plot(axis, f(axis, a / 10, b), label = str(a))
